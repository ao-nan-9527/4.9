...
static int bq27441_battery_full_param_write_cmd(struct chip_bq27541 *chip)
{
	...
	if(read_buf[0] == 0x11  && read_buf[1] == 0x21)
	{
		usleep_range(15000,15000);
		CNTL1_VAL_1[0] = 0x41;
		CNTL1_VAL_1[1] = 0x00;
		bq27541_write_i2c_block(0x00, 2, CNTL1_VAL_1);
		pr_err("%s 0x00 -->write [0x%02x][0x%02x] reset guage 1\n", __func__, CNTL1_VAL_1[0], CNTL1_VAL_1[1]);
		msleep(4500);
	...
	if(read_buf[0] == 0x00  && read_buf[1] == 0x6F)
	{
		usleep_range(15000,15000);
		CNTL1_VAL_1[0] = 0x41;
		CNTL1_VAL_1[1] = 0x00;
		bq27541_write_i2c_block(0x00, 2, CNTL1_VAL_1);
		pr_err("%s 0x00 -->write [0x%02x][0x%02x] reset guage 2\n", __func__, CNTL1_VAL_1[0], CNTL1_VAL_1[1]);
		msleep(4500);
		return BATT_FULL_ERROR;
	}
