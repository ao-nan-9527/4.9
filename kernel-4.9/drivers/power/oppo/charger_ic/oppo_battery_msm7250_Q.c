...
#define FAST_CURRENT_UP_THR 4500
#define FAST_CURRENT_DOWN_THR 3000
#define USBTEMP_FAST_CURRENT_UP_THR 12
#define USBTEMP_FAST_CURRENT_DOWN_THR 6
#define FAST_CURRENT_COOL_DOWN_HIGH 6
#define FAST_CURRENT_COOL_DOWN_LOW 3
...
static void usbtemp_callback(struct timer_list *unused)
{
	struct oppo_chg_chip *chip = g_oppo_chip;

	pinctrl_select_state(chip->normalchg_gpio.pinctrl, chip->normalchg_gpio.dischg_disable);
	oppo_clear_usb_status(USB_TEMP_HIGH);
	chg_err(" usbtemp exit after 4 hours \n");
}

static void usbtemp_exit_init (void)
{
	//init_timer(& usbtemp_timer);
	timer_setup(&usbtemp_timer, usbtemp_callback, 0);
	...
	while (!kthread_should_stop()) {
		...
		if (oppo_usbtemp_condition() == false) {
			...
			if ((USB_57C <= chip->usb_temp_r && chip->usb_temp_r < USB_100C)
				...
				//msleep(delay);
			} else if (abs(chip->icharging) >= FAST_CURRENT_UP_THR){  		//current limit
				if (((chip->usb_temp_r - chip->temperature/10) >= USBTEMP_FAST_CURRENT_UP_THR&& abs(chip->icharging) > FAST_CURRENT_DOWN_THR)
						|| ((chip->usb_temp_l - chip->temperature/10) >= USBTEMP_FAST_CURRENT_UP_THR && abs(chip->icharging) > FAST_CURRENT_DOWN_THR ) ){
					for (i = 0; i < retry_cnt; i++) {
						if ((chip->usb_temp_l - chip->temperature/10) >= USBTEMP_FAST_CURRENT_UP_THR)
							count_r++;
						if ((chip->usb_temp_l - chip->temperature/10) >= USBTEMP_FAST_CURRENT_UP_THR)
							count_l++;
					}
					if (count_r >= retry_cnt || count_l >= retry_cnt) {
						if(chip->cool_down != 0){
							chip->cool_down = oppo_vooc_get_smaller_battemp_cooldown(FAST_CURRENT_COOL_DOWN_LOW,chip->cool_down);
						}else{
							chip->cool_down = FAST_CURRENT_COOL_DOWN_LOW;
						}
						chg_err("usbtemp_cool_down set here, icharging = %d\n",chip->icharging);
						limit_current_flag = true;

						count_r = 0;
						count_l = 0;
					}
				}
			} else if (limit_current_flag == true){
				if(((chip->usb_temp_r - chip->temperature/10) <= USBTEMP_FAST_CURRENT_DOWN_THR&& abs(chip->icharging) < FAST_CURRENT_DOWN_THR)
						|| ((chip->usb_temp_l - chip->temperature/10) <= USBTEMP_FAST_CURRENT_DOWN_THR && abs(chip->icharging) < FAST_CURRENT_DOWN_THR ) ){
					for (i = 0; i < retry_cnt; i++) {          //RETRY_COUNT=3
						if ((chip->usb_temp_l - chip->temperature/10) <= USBTEMP_FAST_CURRENT_DOWN_THR)
							count_r++;
						if ((chip->usb_temp_l - chip->temperature/10) <= USBTEMP_FAST_CURRENT_DOWN_THR)
							count_l++;
					}
					if (count_r >= retry_cnt || count_l >= retry_cnt) {
						if(chip->cool_down != 0){
							chip->cool_down = oppo_vooc_get_smaller_battemp_cooldown(FAST_CURRENT_COOL_DOWN_HIGH,chip->cool_down);
						}else{
							chip->cool_down = FAST_CURRENT_COOL_DOWN_HIGH;
						}
						chg_err("usbtemp_cool_down recover here, icharging = %d\n",chip->icharging);
						limit_current_flag = false;
